...
const String allowedUIDs[] = { "edff252", "5ec8c01" };
const String deviceName = "room two";
...
void setup() {
  ...
  if (Firebase.RTDB.get(&fbdo, "/rooms/roomTwo/expired")) {
...
void openRelayAccess(String source) {
  ...
  Firebase.RTDB.setTimestamp(&fbdo, "/rooms/roomTwo/latestOpen");
...
void enrollFingerprint(int id) {
  ...
  Firebase.RTDB.setInt(&fbdo, "/settings/regFingerTwo", 0);
}
...
void updateLatestOnline() {
  ...
  if (Firebase.RTDB.setTimestamp(&fbdo, "/rooms/roomTwo/latestOnline")) {
...
void checkFirebaseAccess() {
  if (Firebase.ready() && (millis() - lastCheckTime > checkInterval)) {
    ...
    // Baca nilai isOpen
    if (Firebase.RTDB.getInt(&fbdo, "/rooms/roomTwo/isOpen")) {
      int isOpen = fbdo.intData();

      // Baca nilai override
      int overrideCount = 0;
      if (Firebase.RTDB.getInt(&fbdo, "/rooms/roomTwo/override")) {
      ...
      if (isOpen == 1) {
        if (overrideCount >= 3) {
          ...
          overrideCount++;
          if (Firebase.RTDB.setInt(&fbdo, "/rooms/roomTwo/override", overrideCount)) {
...
void checkFingerprintMode() {
  if (Firebase.RTDB.getInt(&fbdo, "/settings/regFingerTwo")) {
    regFingerMode = fbdo.intData();
    Serial.println("regFingerTwo: " + String(regFingerMode));
  } else {
    Serial.println("Gagal baca regFingerTwo: " + fbdo.errorReason());
  }
}
